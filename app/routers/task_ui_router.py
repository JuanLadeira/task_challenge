...
@router.post("/ui/todos", response_class=HTMLResponse)
def create_todo(
    request: Request, 
    session: DBSession,
    content: str = Form(...),
...
@router.put("/ui/todos/{todo_id}", response_class=HTMLResponse)
...
@router.delete("/ui/todos/{todo_id}", response_class=HTMLResponse)
